...
" URL:          https://github.com/hbarcelos/vim-solidity

if exists("b:current_syntax")
  finish
endif

function! ExtendHighlight(group, exts)
  if !exists("*hlget")
    return
  endif

  let l:hl = hlget(a:group, v:true)[0]
  for l:key in keys(a:exts)
    let l:hl[l:key] = hl->get(l:key, {})->extend(a:exts[l:key])
  endfor
...
hi def link solGlobalVar        PreProc
hi def link solGlobalVarMember  PreProc
hi def link solBuiltinFunction  PreProc
...
syn keyword solPragma           pragma skipwhite nextgroup=solPragmaToken,solPragmaExperimental
syn keyword solPragmaToken      contained solidity skipwhite nextgroup=solVersion
syn keyword solPragmaToken      contained experimental skipwhite nextgroup=solPragmaFeatToggle
syn match   solVersion          contained /\v(\^|\>|\>\=|\<|\<\=|\~)?\d+\.\d+\.\d+/ skipwhite nextgroup=solVersion
syn match   solPragmaFeatToggle contained /[^;]*\(;$\)\@!/

hi def link solPragma           Statement
hi def link solPragmaToken      Keyword
hi def link solVersion          Constant
hi def link solPragmaFeatToggle Normal
...
hi def link solImportName     Type
...
syn match   solContractInherit contained /\<\K\k*/ skipwhite skipempty nextgroup=solContractInherit,solContractComma
syn match   solContractComma   contained ',' skipwhite skipempty nextgroup=solContractInherit

hi def link solContract        Keyword
hi def link solContractName    Type
hi def link solContractInherit Type
...
hi def link solStructName Structure

" Enum
syn keyword solEnum         enum skipwhite nextgroup=solEnumName
syn match   solEnumName     contained /\<\K\k*/ skipwhite skipempty

hi def link solEnum     Keyword
hi def link solEnumName Structure
...
" Event
syn match solEventName /\v(<event\_s+)@<=\K\k*>/

hi def link solEventName Function

" Using statements
syn match   solStatement     /\_^\s*using .*/ transparent contains=solUsing,solUsingLibName,solUsingFor,SolUsingAllTypes,solBuiltinType,solUserTpe
syn keyword solUsing         contained using skipwhite skipempty nextgroup=solUsingLibName
syn match   solUsingLibName  contained /\<\K\k*/ nextgroup=solUsingFor
syn keyword solUsingFor      contained for skipwhite skipempty nextgroup=solBuiltinType,solUserType,solUsingAllTypes
syn match   solUsingAllTypes contained /\*/

hi def link solUsing         Keyword
hi def link solUsingLibName  Type
hi def link solUsingFor      Keyword
hi def link solUsingAllTypes Type

" Comment
syn keyword solCommentTodo        contained containedin=solComment,solLineComment TODO FIXME XXX TBD
syn match   solNatspecTag         contained containedin=solComment,solLineComment /\(@title\|@author\|@notice\|@dev\)\>/
syn match   solNatspecTag         contained containedin=solComment,solLineComment /\(@param\|@return\)\>/ skipwhite nextgroup=solNatspecParam
syn match   solNatspecTag         contained containedin=solComment,solLineComment /@inheritdoc\>/ skipwhite nextgroup=solNatspecParam
" We need the lookbehind to match only the first occurence after the tag.
syn match   solNatspecParam       contained /\(@param\s\+\)\@<=\K\k*/
syn match   solNatspecParam       contained /\(@inheritdoc\s\+\)\@<=\K\k*/
" This is a hack! Return param names conventionally start with lower-case letters or and underscore.
" @return might not be named. In that case, the following description needs to start with an upper-case letter, otherwise we'd get a false positive.
